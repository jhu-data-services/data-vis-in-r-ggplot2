...
## Seven parameters of a full ggplot2 visualization:
...
```{r out.width="75%", echo=FALSE, message=FALSE}
ggplot(data = starwars, 
       mapping = aes(x = height, y = birth_year)) +
  geom_point() +
  labs(title = "Mass vs. Birth year of Starwars characters",
       x = "Height (cm)", y = "Birth year")
```
...
.your-turn[
- Open the `data-vis-in-r-ggplot/In-ClassScripts/ggplot2-workshop-exercises.Rmd` RMarkdown file.
...
.footnote[
The materials for this workshop are at:<br> [github.com/jhu-data-services/data-vis-in-r-with-ggplot2](github.com/jhu-data-services/data-vis-in-r-with-ggplot2)
]
...
.your-turn[
- Open the `data-vis-in-r-ggplot/In-ClassScripts/ggplot2-workshop-exercises.Rmd` RMarkdown file.
- Complete the code for `Your turn 2 - Mappings`
  - Modify the included code to plot height vs. birth_year. See `?starwars` for the definition of `birth_year` in this context.
...
.footnote[
The materials for this workshop are at:<br> [github.com/jhu-data-services/data-vis-in-r-ggplot2](github.com/jhu-data-services/data-vis-in-r-ggplot2)
]
...
ggplot(data = starwars, mapping = aes(x = height, y = birth_year)) +
  geom_point(size = 1.5) + 
  labs(title = "Mass vs. Birth year of Starwars characters",
       x = "Birth year", y = "Weight (kg)")
dev.off()

out <- fig %>% image_composite(yoda, offset = "+300+70")
